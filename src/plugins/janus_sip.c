...
static struct janus_json_parameter preaccept_parameters[] = {
		{"srtp", JSON_STRING, 0},
		{"headers", JSON_OBJECT, 0}
};
...
void janus_sip_create_session(janus_plugin_session *handle, int *error) {
	...
	g_atomic_int_set(&session->preaccepted, 0);
...
json_t *janus_sip_query_session(janus_plugin_session *handle) {
	...
	json_object_set_new(info, "preaccepted", json_integer(g_atomic_int_get(&session->preaccepted)));
...
static void *janus_sip_handler(void *data) {
	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "register")) {
		...
		} else if(!strcasecmp(request_text, "preaccept")) {
			...
			janus_mutex_unlock(&session->mutex);
			JANUS_VALIDATE_JSON_OBJECT(root, preaccept_parameters,
									   error_code, error_cause, TRUE,
									   JANUS_SIP_ERROR_MISSING_ELEMENT, JANUS_SIP_ERROR_INVALID_ELEMENT);
			if(error_code != 0)
				goto error;
			...
			if(session->media.require_srtp && !has_srtp) {
				JANUS_LOG(LOG_ERR, "Can't preaccept the call: SDES-SRTP required, but caller didn't offer it\n");
				error_code = JANUS_SIP_ERROR_TOO_STRICT;
				g_snprintf(error_cause, 512, "Can't preaccept the call: SDES-SRTP required, but caller didn't offer it");
				goto error;
			}
			...
			if(notify_events && gateway->events_is_enabled()) {
				json_t *info = json_object();
				json_object_set_new(info, "event", json_string(answer ? "preaccepted" : "preaccepting"));
				if(session->callid)
